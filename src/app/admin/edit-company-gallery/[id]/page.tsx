...
const Edit = () => {
    ...
    const getSingleRecord = async () => {
        try {
            let response = await axios.get(`/api/company-gallery/${id}`);
            if (response.data) {
                setDescription(response.data.description);
                setTitle(response.data.title);
                setCompany(response.data.company);
                setPrevImg(response.data.gallery_img);
                setImage(response.data.gallery_img);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await axios.put(`/api/company-gallery/${id}`, data);
            ...
            if (!response.data.success) {
                ...
                setTimeout(() => {
                    router.push('/admin/company-gallery-list');
                }, 500);
            }
        } catch (error: any) {
