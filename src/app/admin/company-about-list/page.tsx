...
const ItemList = () => {
    ...
    const getDataList = async () => {
        try {
            const response = await axios.get('/api/company-about');
    ...
    const handleDelete = async (id: string) => {
        try {
            const response = await axios.delete(`/api/company-about/${id}`);
                        ...
                        <div className="bg-white px-4 py-5 rounded-3">
                            <div className="d-flex justify-content-between align-items-center mb-5">
                                <h5 className="sec-title position-relative mb-0">
                                    <Link href={'/admin/add-company-about'} className="btn btn-sm btn-primary" style={{ float: 'right' }}> + Add </Link><br />
                                </h5>
                                <div className="w-50">
                                ...
                                <tbody>
                                    {currentItems.map((item, i) => (
                                        ...
                                        <tr key={item._id}>
                                            ...
                                            <td>
                                                <div className="d-flex align-items-center">
                                                    <Image
                                                        ...
                                                        src={!item.about_img ? '/images/placeholder.png' : item.about_img}
                                            ...
                                            <td>{item.otherDesc ? ReactHtmlParser(item.otherDesc as string) : null}</td>
                                            <td>
                                                <div>
                                                    <Link href={`/admin/edit-company-about/${item._id}`}>
