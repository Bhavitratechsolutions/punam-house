...
const Edit: React.FC = () => {
    ...
    const getSingleRecord = async () => {
        try {
            let response = await axios.get(`/api/about-brief/${id}`);
            if (response.data) {
                setTitle(response.data.title);
                setHeading(response.data.heading);
                setDescription(response.data.description);
                setSubtitle(response.data.subTitle);
                setSubDesc(response.data.subDesc);
                setSubtitleS(response.data.subTitleS);
                setSubDescS(response.data.subDescS);
                setPrevImg(response.data.about_image);
                setImage(response.data.about_image);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            let response = await axios.put(`/api/about-brief/${id}`, data);
            ...
            if (axios.isAxiosError(error) && error.response) {
                // Use API error response if available
                const errorMessage = error.response.data.message || 'Error updating record';
                toast.error(`Error: ${errorMessage}`);
            } else {
