...
const Edit = () => {
    ...
    const getSingleRecord = async () => {
        try {
            let response = await axios.get(`/api/company-product/${id}`);
            if (response.data) {
                setDescription(response.data.description);
                setTitle(response.data.title);
                setCompany(response.data.company);
                setPrevImg(response.data.product_img);
                setImage(response.data.product_img);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await axios.put(`/api/company-product/${id}`, data);
            ...
            if (!response.data.success) {
                ...
                setTimeout(() => {
                    router.push('/admin/company-product-list');
                }, 500);
            }
        } catch (error: any) {
